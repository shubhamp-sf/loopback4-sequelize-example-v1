...
export class ProgrammingLanguangeController {
  ...
  @response(200, {
    ...
    content: {'application/json': {schema: getModelSchemaRef(ProgrammingLanguage)}},
  })
  async create(
    @requestBody({
      content: {
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: {
          type: 'array',
          items: getModelSchemaRef(ProgrammingLanguage, {includeRelations: true}),
        },
      },
    },
  })
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: getModelSchemaRef(ProgrammingLanguage, {includeRelations: true}),
      },
    },
  })
  async findById(
    @param.path.number('id') id: number,
    @param.filter(ProgrammingLanguage, {exclude: 'where'}) filter?: FilterExcludingWhere<ProgrammingLanguage>
  ): Promise<ProgrammingLanguage> {
